...
public class Main {
    public static void main(String[] args) {
        ...
        // Tạo một Map để lưu trữ từ và số lần xuất hiện của từ
        Map<String, Integer> wordCountMap = new HashMap<>();

        // Chia câu thành các từ bằng cách sử dụng phương thức split()
        String[] words = sentence.split("\\s+");

        // Đếm số lần xuất hiện của mỗi từ và lưu vào Map
        for (String word : words) {
            // Loại bỏ các ký tự đặc biệt và chuyển đổi thành chữ thường để đảm bảo tính đồng nhất
            word = word.replaceAll("[^a-zA-Z]", "").toLowerCase();

            // Kiểm tra từ đã tồn tại trong Map hay chưa
            if (wordCountMap.containsKey(word)) {
                // Nếu từ đã tồn tại, tăng số lần xuất hiện lên 1
                wordCountMap.put(word, wordCountMap.get(word) + 1);
            } else {
                // Nếu từ chưa tồn tại, thêm vào Map với số lần xuất hiện là 1
